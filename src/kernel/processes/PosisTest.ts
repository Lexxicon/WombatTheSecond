...
interface POSISTestBaseProcessMemory {
  started?: number;
  lastTick?: number;
...
class POSISTestBaseProcess implements IPosisProcess {
  ...
  public get tmemory(): POSISTestBaseProcessMemory {
    return this.memory as POSISTestBaseProcessMemory;
  }

  public run(): void {
    const kernel: IPosisKernel = this.context.queryPosisInterface("baseKernel") as IPosisKernel;
    let fatal = false;
    ...
    if (this.imageName !== POSISTestBaseProcess.ImageName) {
      this.log.error(
        `${this.testName}: 'imageName' not matching,
         expected: '${POSISTestBaseProcess.ImageName}', actual '${this.imageName}'`);
    }
    ...
    this.log.info(`${this.testName}: started on ${this.tmemory.started},
     running for ${Game.time - this.tmemory.started!}`);
...
export const bundle: IPosisBundle<{}> = {
  ...
  rootImageName: POSISTestBaseProcess.ImageName
};
