...
module.exports = function (logAmount, stringFilter) {
  return through2.obj(function (file, enc, next) {
    if (logAmount && logAmount > 2) {
      log(`>>> flattener starting file '${colors.cyan(path.dirname(file.relative) + path.sep + path.basename(file.path))}'`);
    }
    ...
    if (!file.isDirectory() && !file.isNull() && !file.isStream()) {
      try {
              ...
              if (expr.arguments.length) {
                ...
                if (arg.type == 'Literal' && arg.value[0] == '.') {
                  ...
                  if (logAmount && logAmount > 1) {
                    log(`> in file '${colors.cyan(path.dirname(file.relative) + path.sep + path.basename(file.path))}', flattened path '${colors.cyan(expr.arguments[0].value)}' into '${colors.cyan(result)}'`);
                  }
                  result = result.replace(/[.](ts|js)/g, '')
                  expr.arguments[0] = arg.raw.charAt(0) + result + arg.raw.charAt(0);
                } else {
                  log(`> Non Literal argument for 'require' in '${colors.cyan(path.dirname(file.relative) + path.sep + path.basename(file.path))}' location: ${arg.loc.start.line}:${arg.loc.start.column}`)
                }
              } else {
                if (logAmount && logAmount > 2) {
        ...
        if (logAmount && logAmount > 0) {
          log(`>> flattened file '${colors.cyan(path.dirname(file.relative) + path.sep + path.basename(file.path))}' into '${colors.cyan(newName)}'`);
        }
